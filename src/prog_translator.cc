...
void ProgTranslator::ToProg() {
  ...
  n_cols = arcs.size() + n_req_nodes;
  m_.setDimensions(0, n_cols);

  bool req;
  std::vector<SupplyGroup::Ptr>& sgs = g->supply_groups();
  for (i = 0; i != sgs->size(); i++) {
    req = false;
    XlateGrp_(sgs[i].get(), req);
  }

  std::vector<RequestGroup::Ptr>& rgs = g->request_groups();
  ...
  // load er up!
  si->loadProblem(m_, &col_lbs_[0], &col_ubs_[0], &obj_coeffs_[0],
                  &row_lbs_[0], &row_ubs_[0]);

  if (excl_) {
    for (i = 0; i != arcs.size(); i++) {
      if (arcs[i].exclusive) {
        iface_->setInteger(g_->arc_ids()[arcs[i]]);
      }
    }
  }
}
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
void Proreqranslator::XlateGrp_(ExchangeNodeGroup* grp, bool req) {
  ...
  for (i = 0; i != nodes.size(); i++) {
    ...
    for (cap_it = ucap_map.begin(); cap_it != ucap_map.end(); ++cap_it) {
      ...
      int arc_id = g_->arc_id()[a];

      // add each unit capacity coefficient
      for (j = 0; j != ucaps.size(); j++) {
      ...
      double pref = nodes[i].pref[a];
      obj_coeffs_[arc_id] = a.exclusive ? pref * excl_val : pref;
      col_lb_.push_back(0);
      col_ub_[arc_id] = a.exclusive ? 1 : inf;
    }
