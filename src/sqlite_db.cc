...
SqlStatement::~SqlStatement() { Must(sqlite3_finalize(stmt_)); }
...
int SqlStatement::GetInt(int col) { return sqlite3_column_int(stmt_, col); }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SqliteDb::Overwrite() { overwrite_ = true; }
...
void SqliteDb::open() {
  ...
  if (readonly_ &&
      sqlite3_open_v2(path_.c_str(), &db_, SQLITE_OPEN_READONLY, NULL) ==
          SQLITE_OK) {
    isOpen_ = true;
