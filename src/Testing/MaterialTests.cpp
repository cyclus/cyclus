...
class SubVect: public IsoVector {
  public:
    int decay_count;
    double time_decayed;
    SubVect() {
    ...
    virtual void decay(double delta) {
      decay_count++;
      time_decayed += delta;
...
class SubMaterial : public Material {
  public:
    IsoVector* vect;
    virtual IsoVector& isoVector() {return *vect;}
};

//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -    
TEST_F(MaterialTest, DecayLogistics) {
  SubMaterial* mat = new SubMaterial;
  mat->vect = new SubVect();
  ...
  SubVect* usedVect = (SubVect*)(mat->vect);
  EXPECT_EQ(usedVect->decay_count, duration / decay_interval);
  EXPECT_EQ(usedVect->time_decayed, duration / decay_interval * decay_interval);
}
