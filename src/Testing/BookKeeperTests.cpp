    ...
    virtual void SetUp(){
      ...
      for (int row=0; row<nrows; row++){
        ...
        for (int col=0; col<ncols; col++){ 
          test_data_int2[col]=test_data_int1 ;
          test_data_dbl2[col]=test_data_dbl1 ;
          test_data_str2[col]=test_data_str1 ; 
          for (int layer=0; layer<nlayers; layer++){ 
            test_data_int3[layer]= test_data_int2  ;
            test_data_dbl3[layer]= test_data_dbl2  ;
            test_data_str3[layer]= test_data_str2 ; 
          };
        };
      };
...
TEST_F(BookKeeperTest, ReadWrite3DIntegers) {
  ...
  ASSERT_EQ(      test_out_int3[0][1][n],                     n ); 
  BI->closeDB();
}
...
TEST_F(BookKeeperTest, ReadWrite2DDoubles) {
  ...
  ASSERT_NEAR(     test_out_dbl2[n][n],        0.1*n,      0.001 );  
  BI->closeDB();
}
TEST_F(BookKeeperTest, ReadWrite3DDoubles) {
  ...
  EXPECT_NEAR(    test_out_dbl3[n][n][n],      0.1*n,      0.001 );  
  BI->closeDB();
}
