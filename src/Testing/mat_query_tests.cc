...
TEST(MatQueryTests, MassAndMoles) { 
  ...
  cyclus::Material::Ptr m = cyclus::Material::Create(4.0, c);
...
TEST(MatQueryTests, AlmostEq) { 
  ...
  cyclus::Material::Ptr m1 = cyclus::Material::Create(4.0, c);
  cyclus::MatQuery mq(m1);

  cyclus::Material::Ptr m2 = cyclus::Material::Create(4.0, c);
  EXPECT_TRUE(mq.AlmostEq(m2, 0));

  c = cyclus::Composition::CreateFromMass(v);
  cyclus::Material::Ptr m3 = cyclus::Material::Create(4.0, c);
  EXPECT_TRUE(mq.AlmostEq(m3, 0));

  v[1008] += 0.99 * cyclus::eps_rsrc();
  c = cyclus::Composition::CreateFromMass(v);
  cyclus::Material::Ptr m4 = cyclus::Material::Create(4.0, c);
  EXPECT_FALSE(mq.AlmostEq(m4, 0));
  EXPECT_TRUE(mq.AlmostEq(m4, cyclus::eps_rsrc()));

  v[1008] += 4.0 * cyclus::eps_rsrc();
  c = cyclus::Composition::CreateFromMass(v);
  cyclus::Material::Ptr m5 = cyclus::Material::Create(4.0, c);
