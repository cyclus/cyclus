...
int differentAgent(OsiSolverInterface& lhs, OsiSolverInterface& rhs,
  ...
  if (numberRows != rhs.getNumRows() || numberColumns != rhs.getNumCols()) {
    if (printStuff)
      printf(
          "** Mismatch on size, this has %d rows, %d columns - rhs has %d "
          "rows, %d columns\n",
  ...
  for (int i = 0; i < numberColumns; i++) {
    if (lhs.isInteger(i)) {
      if (!rhs.isInteger(i)) numberErrors1++;
    } else {
      if (rhs.isInteger(i)) numberErrors2++;
    }
  }
  if (numberErrors1 || numberErrors2) {
    if (printStuff)
      printf(
          "** Mismatch on integers, %d (this int, rhs not), %d (this not rhs "
          "int)\n",
  ...
  for (int i = 0; i < numberRows; i++) {
    if (!tolerance(rowLower[i], rowLower2[i])) numberDifferentL++;
    if (!tolerance(rowUpper[i], rowUpper2[i])) numberDifferentU++;
  }
  ...
  for (int i = 0; i < numberColumns; i++) {
    if (!tolerance(columnLower[i], columnLower2[i])) numberDifferentL++;
    if (!tolerance(columnUpper[i], columnUpper2[i])) numberDifferentU++;
    if (!tolerance(objective[i], objective2[i])) numberDifferentO++;
  }
  ...
  if (matrix->getNumElements() == rhs.getNumElements()) {
    if (!matrix->isEquivalent(*matrix2, tolerance)) {
      returnCode += 100;
      if (printStuff) printf("Two matrices are not same\n");
    }
  } else {
    returnCode += 200;
    if (printStuff)
