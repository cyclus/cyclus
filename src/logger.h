...
class Logger {
 public:
  ...
  static void SetReportLevel(LogLevel level) { report_level = level; }
  ...
  /// to a reference from a function return yet.
  static void SetNoAgent(bool na) { no_agent = na; }
  ...
  /// This is here because Cython cannot assign
  /// to a reference from a function return yet.
  static void SetNoMem(bool nm) { no_agent = nm; }
  ...
  /// `BAD_LEVEL` is returned.  This method is primarily intended for
  /// translating LOG(level) statement levels into appropriate strings for
  /// output to stdout.
 ...
 private:
  ...
  /// This should be equal to or greater than the length of the longest LogLevel
  /// enum value converted to a string (i.e. `LEV_WARN` is the longest with 11
  /// characters).
