...
void Timer::Initialize(Context* ctx, SimInfo si) {
  if (si.m0 < 1 || si.m0 > 12) {
    ...
    Warn<VALUE_WARNING>("Invalid year0; the first man-made nuclear reactor was build in 1942");
  } else if (si.y0 > 2063) {
    Warn<VALUE_WARNING>("Invalid year0; why start a simulation after we've got warp drive?: http://en.wikipedia.org/wiki/Warp_drive#Development_of_the_backstory");
