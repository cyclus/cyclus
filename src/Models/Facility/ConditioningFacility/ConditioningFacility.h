...
/**
   ...
   This facility accepts material that must be conditioned into a waste 
   form.  
   The ConditioningFacility class inherits from the FacilityModel class 
   ...
   - string fileformat : Currently supported formats for wasteform 
   tables include hdf5 and csv 
   - string datafile : The filename of the file containing the wasteform 
   tables  
   ...
   On the tick the ConditioningFacility makes requests for each  of the 
   waste commodities (streams) that its table addresses. It also makes 
   offers of any conditioned waste it contains. 
    
   @subsection tock handleTock 
    
   On the tock the ConditioningFacility first prepares transactions by 
   preparing and sending material from its stocks all material in its 
   stocks up to its monthly processing  capacity. 
    
   @subsection receivemessage receiveMessage 
    
   The ConditioningFacility throws an exception, as it doesn't deal with 
   messages.  
   ...
   Puts the material it has recieved in the stocks, to be conditioned on 
   the tick.  
 */
 ...
 */
  ...
  /**
     The integer length (number of rows) of the datafile/med/loading/ 
   */
  int stream_len_;

  /**
     The vector of stream structs that holds the data in the datafile 
   */
  ...
  /**
     Loading density table. Rows are waste streams, columns are waste 
   */
  ...
  /**
     Processing capacity per time unit not yet consumed this tick (in 
   */
 ...
 private :
  /**
     Condition the material provided according to the rules for this 
     commodity  
  ...
  /**
     ...
     @return the stream where the commodity matches the id in the 
   */
  ...
  /**
     ...
     @param incommod is a string naming the commodity associated with 
     the raw resource. @param outcommod is a string naming the commodity 
   */
