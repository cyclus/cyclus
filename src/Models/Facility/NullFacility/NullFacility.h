...
/**
   ...
   The NullFacility is a facility type in *Cyclus* capable of converting 
   a constant amount of a particular commodity type into the same amount 
   of a different commodity type.  A NullFacility generates material of 
   the output commodity type any time that it has been provided with 
   material of the input commodity type. It continuously makes requests 
   for that commodity type and offers the output commodity type when it 
   has some in stock.  
   ...
   vary, but typically kg/month). *Question:* Do we want to allow this 
   to be infinite? 
   - int startDate: The date on which the facility begins to operate 
   (months). - int lifeTime: The length of time that the facility 
   operates (months). - std::string inCommod: The type of commodity that 
   this facility accepts. - std::string outCommod: The type of commodity 
   that this facility produces. 
    
   @section optionalParams Optional Parameters 
   NullFacility behavior may also be specified with the following 
   ...
   The NullFacility starts operation when the simulation reaches the 
   month specified as the startDate. 
    
   Each month, the NullFacility makes a request for the inCommod 
   commodity type at a rate corresponding to the NullFacility capacity 
   less the amount it currently has in its stocks (which begins the 
   simulation empty). It also makes an offer of any materials of the 
   outCommod type in its inventory (which also begins the simulation 
   empty).  
   If a request is matched with an offer, the NullFacility executes that 
   order by adding the quantity to its stocks. It then proceeds to 
   convert the material into the outCommod commod type at the rate 
   defined by its capacity, leaving the isotopic composition and all 
   other material attributes the same. At the end of the month, it adds 
   the quantity of outCommod it has produced to its inventory and offers 
   that cumulative amount to the appropriate market. 
    
   If an offer is matched with a request, the NullFacility executes that 
   order by subtracting the quantity from its stocks and sending that 
   amount to the requesting facility. 
